...
export default function DocumentList(){
  ...
  useEffect(()=>{ api<Meeting[]>('/document').then(setData).catch(console.error) }, [])
  return (
    <div>
      <h1 className="text-xl font-semibold mb-4">Document</h1>
      <ul className="space-y-2">
        {data.map(m => (
          <li key={m.id} className="p-3 border rounded-lg">
            <a className="hover:underline" href={`/document/${m.id}`}>{m.title}</a>
