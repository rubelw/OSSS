...
type Meeting = { id:number; title:string; start_at:string; location:string; agenda_items:AgendaItem[] }

export default function MeetingDetail(){
  const { id } = useParams()
  const [m,setM] = useState<Meeting | null>(null)
  useEffect(()=>{ api<Meeting>(`/meetings/${id}`).then(setM).catch(console.error) }, [id])
