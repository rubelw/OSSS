...
type Communications = { id:number; title:string; start_at:string; location:string; agenda_items:AgendaItem[] }

export default function CommunicationsDetail(){
  const { id } = useParams()
  const [m,setM] = useState<Communications | null>(null)
  useEffect(()=>{ api<Communications>(`/communications/${id}`).then(setM).catch(console.error) }, [id])
