...
type Planning = { id:number; title:string; start_at:string; location:string; agenda_items:AgendaItem[] }

export default function PlanningDetail(){
  const { id } = useParams()
  const [m,setM] = useState<Planning | null>(null)
  useEffect(()=>{ api<Planning>(`/planning/${id}`).then(setM).catch(console.error) }, [id])
