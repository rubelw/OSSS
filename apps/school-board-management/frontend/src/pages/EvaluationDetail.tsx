...
type Evaluation = { id:number; title:string; start_at:string; location:string; agenda_items:AgendaItem[] }

export default function EvaluationDetail(){
  const { id } = useParams()
  const [m,setM] = useState<Evaluation | null>(null)
  useEffect(()=>{ api<Evaluation>(`/evaluation/${id}`).then(setM).catch(console.error) }, [id])
