# Auto-generated from QueryData handler mode="assets"
...
def _coerce_list(payload: Any, *, label: str) -> List[Dict[str, Any]]:
    ...
    if isinstance(payload, dict):
        for k in ("items", "data", "results", "rows", "assets"):
            v = payload.get(k)
            if isinstance(v, list):
                return [x for x in v if isinstance(x, dict)]
        logger.warning("[%s_table] %s payload dict had no list key. keys=%s", "assets", label, list(payload.keys())[:30])
        return []
    logger.warning("[%s_table] %s payload unexpected type=%s", "assets", label, type(payload).__name__)
    return []
...
async def _fetch_rows(*, skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    if handler is None:
        logger.error("[%s_table] No QueryData handler registered for mode=%r", "assets", "assets")
        return []

    result = await handler.fetch(ctx=None, skip=skip, limit=limit)
    rows = result.get("rows") or result.get("assets") or []
    return _coerce_list(rows, label="assets")
...
async def run_assets_table_structured(
    *,
    ...
    logger.info("[%s_table] called filters=%s", "assets", filters.model_dump() if filters else None)

    rows = await _fetch_rows(skip=skip, limit=limit)
    rows = rows[:SAFE_MAX_ROWS]

    summary = [
        f"I found {len(rows)} assets records.",
        "",
        "Sample (first 50):",
        "",
...
async def run_assets_table_markdown_only(
    *,
    ...
    result = await run_assets_table_structured(filters=filters, session_id=session_id, skip=skip, limit=limit)
    return result["reply"]
