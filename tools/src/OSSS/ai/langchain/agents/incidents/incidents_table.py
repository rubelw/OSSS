# Auto-generated from QueryData handler mode="incidents"
...
def _coerce_list(payload: Any, *, label: str) -> List[Dict[str, Any]]:
    ...
    if isinstance(payload, dict):
        ...
        logger.warning("[%s_table] %s payload dict had no list key. keys=%s", "incidents", label, list(payload.keys())[:30])
        return []
    logger.warning("[%s_table] %s payload unexpected type=%s", "incidents", label, type(payload).__name__)
    return []
...
async def _fetch_rows(*, skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    if handler is None:
        logger.error("[%s_table] No QueryData handler registered for mode=%r", "incidents", "incidents")
        return []
...
async def run_incidents_table_structured(
    *,
    ...
    logger.info("[%s_table] called filters=%s", "incidents", filters.model_dump() if filters else None)

    rows = await _fetch_rows(skip=skip, limit=limit)
    rows = rows[:SAFE_MAX_ROWS]

    summary = [
        f"I found {len(rows)} incidents records.",
        "",
        "Sample (first 50):",
        "",
...
async def run_incidents_table_markdown_only(
    *,
    ...
    result = await run_incidents_table_structured(filters=filters, session_id=session_id, skip=skip, limit=limit)
    return result["reply"]
