...
def _coerce_list(payload: Any, *, label: str) -> List[Dict[str, Any]]:
    ...
    if isinstance(payload, dict):
        ...
        logger.warning("[%s_table] %s payload dict had no list key. keys=%s", "earning_codes", label, list(payload.keys())[:30])
        return []
    logger.warning("[%s_table] %s payload unexpected type=%s", "earning_codes", label, type(payload).__name__)
    return []
...
async def _fetch_rows(*, skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    if handler is None:
        logger.error("[%s_table] No QueryData handler registered for mode=%r", "earning_codes", "earning_codes")
        return []
...
async def run_earning_codes_table_structured(
    *,
    ...
    logger.info("[%s_table] called filters=%s", "earning_codes", filters.model_dump() if filters else None)
...
async def run_earning_codes_table_markdown_only(
    *,
    ...
    result = await run_earning_codes_table_structured(filters=filters, session_id=session_id, skip=skip, limit=limit)
    return result["reply"]
