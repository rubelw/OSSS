...
# ---- CONFIG ----
OUTPUT_FILE = "training_generated.json"
...
def humanize(name: str) -> str:
    """Convert snake_case → human readable names"""
    return name.replace("_", " ")


def build_training_examples() -> List[Dict]:
    tables = [t.strip() for t in TABLE_NAMES_RAW.splitlines() if t.strip()]
    data = []

    for table in tables:
        pretty = humanize(table)
        for tmpl in TEMPLATES:
            data.append({
                "text": tmpl.format(pretty),
                "intent": "action",
                "domain": "data_systems",
                "topics": [table]
            })
...
if __name__ == "__main__":
    ...
    print(f"✔ Wrote {len(examples)} training records to {OUTPUT_FILE}")
