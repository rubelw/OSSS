...
class PolicyVersion(UUIDMixin, TimestampMixin, Base):
    ...
    policy_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("policies.id", ondelete="CASCADE"), nullable=False)
    version_no: Mapped[int] = mapped_column(Integer, default=1, nullable=False)
    content: Mapped[Optional[str]] = mapped_column(Text)
    effective_date: Mapped[Optional["date"]] = mapped_column(Date)  # type: ignore
    supersedes_version_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("policy_versions.id", ondelete="SET NULL"))
...
class PolicyLegalRef(UUIDMixin, Base):
    ...
    policy_version_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("policy_versions.id", ondelete="CASCADE"), nullable=False)
...
class PolicyWorkflowStep(UUIDMixin, Base):
    ...
    workflow_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("policy_workflows.id", ondelete="CASCADE"), nullable=False)
...
class PolicyApproval(UUIDMixin, Base):
    ...
    policy_version_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("policy_versions.id", ondelete="CASCADE"), nullable=False)
    step_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("policy_workflow_steps.id", ondelete="CASCADE"), nullable=False)
...
class PolicyPublication(Base):
    ...
    policy_version_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("policy_versions.id", ondelete="CASCADE"), primary_key=True)
...
class PolicySearchIndex(Base):
    ...
    policy_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("policies.id", ondelete="CASCADE"), primary_key=True)
