...
class EvaluationSection(UUIDMixin, Base):
    ...
    template_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("evaluation_templates.id", ondelete="CASCADE"), nullable=False)
...
class EvaluationQuestion(UUIDMixin, Base):
    ...
    section_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("evaluation_sections.id", ondelete="CASCADE"), nullable=False)
...
class EvaluationAssignment(UUIDMixin, Base):
    ...
    cycle_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("evaluation_cycles.id", ondelete="CASCADE"), nullable=False)
    subject_user_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    evaluator_user_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    template_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("evaluation_templates.id", ondelete="RESTRICT"), nullable=False)
    status: Mapped[Optional[str]] = mapped_column(String(32))

class EvaluationResponse(UUIDMixin, Base):
    __tablename__ = "evaluation_responses"
    assignment_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("evaluation_assignments.id", ondelete="CASCADE"), nullable=False)
    question_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("evaluation_questions.id", ondelete="CASCADE"), nullable=False)
...
class EvaluationSignoff(UUIDMixin, Base):
    ...
    assignment_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("evaluation_assignments.id", ondelete="CASCADE"), nullable=False)
    signer_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
...
class EvaluationFile(Base):
    ...
    assignment_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("evaluation_assignments.id", ondelete="CASCADE"), primary_key=True)
...
class EvaluationReport(UUIDMixin, Base):
    ...
    file_id: Mapped[Optional[UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("files.id", ondelete="SET NULL"))
