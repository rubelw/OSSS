    ...
    try {
      const res = await fetch(`${A2A_BASE}/admin/trigger`, {
        ...
        body: JSON.stringify({
          ...
          input: newInput,
          skill: newSkill || null, // <-- NEW
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || data.detail || "Failed to trigger run");
      setNewInput("");
      // keep agent + skill
      fetchRuns();
    } catch (e: any) {
            ...
            <div className="space-y-2">
              <div className="flex gap-2">
                ...
                <select
                  ...
                  className="w-40 rounded-xl bg-slate-950/80 border border-slate-700 px-2 py-1.5 text-sm text-slate-100 focus:outline-none focus:ring-1 focus:ring-sky-500"
                >
