...
async def _fetch_meal_accounts(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/meal_accounts"
    ...
    try:
        ...
        logger.exception("Error calling meal_accounts API")
        raise QueryDataError(
            f"Error querying meal_accounts API: {e}",
...
def _build_meal_accounts_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class MealAccountsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (meal_accounts)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_meal_accounts(skip=skip, limit=limit)
        return {"rows": rows, "meal_accounts": rows}
