...
async def _fetch_plan_assignments(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/plan_assignments"
    ...
    try:
        ...
        logger.exception("Error calling plan_assignments API")
        raise QueryDataError(
            f"Error querying plan_assignments API: {e}",
...
class PlanAssignmentsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (plan_assignments)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_plan_assignments(skip=skip, limit=limit)
        return {"rows": rows, "plan_assignments": rows}
