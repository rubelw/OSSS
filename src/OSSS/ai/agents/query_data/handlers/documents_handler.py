...
async def _fetch_documents(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/documents"
    ...
    try:
        ...
        logger.exception("Error calling documents API")
        raise QueryDataError(
            f"Error querying documents API: {e}",
            documents_url=url,
        ) from e
...
def _build_documents_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_documents_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class DocumentsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "documents": rows}
