...
async def _fetch_gl_segment_values(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/gl_segment_values"
    ...
    try:
        ...
        logger.exception("Error calling gl_segment_values API")
        raise QueryDataError(
            f"Error querying gl_segment_values API: {e}",
...
def _build_gl_segment_values_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_gl_segment_values_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class GlSegmentValuesHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "gl_segment_values": rows}
