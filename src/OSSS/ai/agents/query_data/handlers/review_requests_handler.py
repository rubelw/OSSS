...
class ReviewRequestsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        rows = await _fetch_review_requests(skip=skip, limit=limit)
        return {"rows": rows, "review_requests": rows}

    def to_markdown(self, rows: List[Dict[str, Any]]) -> str:
        return _build_review_requests_markdown_table(rows)

    def to_csv(self, rows: List[Dict[str, Any]]) -> str:
        return _build_review_requests_csv(rows)


# register on import
register_handler(ReviewRequestsHandler())
