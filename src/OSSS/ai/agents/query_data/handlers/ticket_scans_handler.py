...
async def _fetch_ticket_scans(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling ticket_scans API")
        raise QueryDataError(
            f"Error querying ticket_scans API: {e}",
            ticket_scans_url=url,
        ) from e
...
class TicketScansHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (ticket_scans)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_ticket_scans(skip=skip, limit=limit)
        return {"rows": rows, "ticket_scans": rows}
