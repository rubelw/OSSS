...
logger = logging.getLogger("OSSS.ai.agents.query_data.department_position_index")

API_BASE = "http://host.containers.internal:8081"


async def _fetch_department_position_index(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/department_position_index"
    ...
    try:
        ...
        logger.exception("Error calling department_position_index API")
        raise QueryDataError(
            f"Error querying department_position_index API: {e}",
...
def _build_department_position_index_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_department_position_index_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class DepartmentPositionIndexHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "department_position_index": rows}
