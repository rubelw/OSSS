...
async def _fetch_gl_account_balances(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/gl_account_balances"
    ...
    try:
        ...
        logger.exception("Error calling gl_account_balances API")
        raise QueryDataError(
            f"Error querying gl_account_balances API: {e}",
...
def _build_gl_account_balances_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_gl_account_balances_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class GlAccountBalancesHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "gl_account_balances": rows}
