...
async def _fetch_person_contacts(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/person_contacts"
    ...
    try:
        ...
        logger.exception("Error calling person_contacts API")
        raise QueryDataError(
            f"Error querying person_contacts API: {e}",
...
class PersonContactsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (person_contacts)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_person_contacts(skip=skip, limit=limit)
        return {"rows": rows, "person_contacts": rows}
