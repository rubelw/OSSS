...
async def _fetch_part_locations(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/part_locations"
    ...
    try:
        ...
        logger.exception("Error calling part_locations API")
        raise QueryDataError(
            f"Error querying part_locations API: {e}",
...
class PartLocationsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (part_locations)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_part_locations(skip=skip, limit=limit)
        return {"rows": rows, "part_locations": rows}
