...
async def _fetch_policies(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/policies"
    ...
    try:
        ...
        logger.exception("Error calling policies API")
        raise QueryDataError(
            f"Error querying policies API: {e}",
            policies_url=url,
        ) from e
...
def _build_policies_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class PoliciesHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (policies)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_policies(skip=skip, limit=limit)
        return {"rows": rows, "policies": rows}
