...
async def _fetch_goals(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/goals"
    ...
    try:
        ...
        logger.exception("Error calling goals API")
        raise QueryDataError(
            f"Error querying goals API: {e}",
            goals_url=url,
        ) from e
...
def _build_goals_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_goals_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
