...
async def _fetch_projects(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling projects API")
        raise QueryDataError(
            f"Error querying projects API: {e}",
            projects_url=url,
        ) from e
...
class ProjectsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (projects)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_projects(skip=skip, limit=limit)
        return {"rows": rows, "projects": rows}
