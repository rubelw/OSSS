...
async def _fetch_gl_segments(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/gl_segments"
    ...
    try:
        ...
        logger.exception("Error calling gl_segments API")
        raise QueryDataError(
            f"Error querying gl_segments API: {e}",
            gl_segments_url=url,
        ) from e
...
def _build_gl_segments_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_gl_segments_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class GlSegmentsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "gl_segments": rows}
