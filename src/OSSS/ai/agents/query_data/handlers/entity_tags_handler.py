...
async def _fetch_entity_tags(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/entity_tags"
    ...
    try:
        ...
        logger.exception("Error calling entity_tags API")
        raise QueryDataError(
            f"Error querying entity_tags API: {e}",
            entity_tags_url=url,
        ) from e
...
def _build_entity_tags_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_entity_tags_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class EntityTagsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "entity_tags": rows}
