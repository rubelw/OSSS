...
logger = logging.getLogger("OSSS.ai.agents.query_data.meeting_documents")

API_BASE = "http://host.containers.internal:8081"


async def _fetch_meeting_documents(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/meeting_documents"
    ...
    try:
        ...
        logger.exception("Error calling meeting_documents API")
        raise QueryDataError(
            f"Error querying meeting_documents API: {e}",
...
def _build_meeting_documents_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class MeetingDocumentsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (meeting_documents)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_meeting_documents(skip=skip, limit=limit)
        return {"rows": rows, "meeting_documents": rows}
