...
async def _fetch_fiscal_years(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/fiscal_years"
    ...
    try:
        ...
        logger.exception("Error calling fiscal_years API")
        raise QueryDataError(
            f"Error querying fiscal_years API: {e}",
...
def _build_fiscal_years_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_fiscal_years_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class FiscalYearsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "fiscal_years": rows}
