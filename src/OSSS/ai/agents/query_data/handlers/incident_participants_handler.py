...
async def _fetch_incident_participants(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling incident_participants API")
    ...
    if not isinstance(data, list):
        raise QueryDataError(
            f"Unexpected incident_participants payload type: {type(data)!r}",
            incident_participants_url=url,
        )
    return data


def _build_incident_participants_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class IncidentParticipantsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "incident_participants": rows}
