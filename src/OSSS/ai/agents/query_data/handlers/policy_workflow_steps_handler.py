...
async def _fetch_policy_workflow_steps(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/policy_workflow_steps"
    ...
    try:
        ...
        logger.exception("Error calling policy_workflow_steps API")
        raise QueryDataError(
            f"Error querying policy_workflow_steps API: {e}",
...
def _build_policy_workflow_steps_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class PolicyWorkflowStepsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (policy_workflow_steps)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_policy_workflow_steps(skip=skip, limit=limit)
        return {"rows": rows, "policy_workflow_steps": rows}
