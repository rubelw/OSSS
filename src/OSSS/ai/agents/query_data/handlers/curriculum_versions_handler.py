...
async def _fetch_curriculum_versions(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/curriculum_versions"
    ...
    try:
        ...
        logger.exception("Error calling curriculum_versions API")
        raise QueryDataError(
            f"Error querying curriculum_versions API: {e}",
...
def _build_curriculum_versions_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_curriculum_versions_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class CurriculumVersionsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "curriculum_versions": rows}
