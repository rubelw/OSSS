...
async def _fetch_document_versions(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/document_versions"
    ...
    try:
        ...
        logger.exception("Error calling document_versions API")
        raise QueryDataError(
            f"Error querying document_versions API: {e}",
...
def _build_document_versions_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_document_versions_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class DocumentVersionsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "document_versions": rows}
