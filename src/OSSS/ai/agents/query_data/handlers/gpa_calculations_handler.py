...
async def _fetch_gpa_calculations(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/gpa_calculations"
    ...
    try:
        ...
        logger.exception("Error calling gpa_calculations API")
        raise QueryDataError(
            f"Error querying gpa_calculations API: {e}",
...
def _build_gpa_calculations_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_gpa_calculations_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class GpaCalculationsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "gpa_calculations": rows}
