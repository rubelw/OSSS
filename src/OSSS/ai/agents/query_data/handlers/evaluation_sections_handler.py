...
async def _fetch_evaluation_sections(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/evaluation_sections"
    ...
    try:
        ...
        logger.exception("Error calling evaluation_sections API")
        raise QueryDataError(
            f"Error querying evaluation_sections API: {e}",
...
def _build_evaluation_sections_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_evaluation_sections_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class EvaluationSectionsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "evaluation_sections": rows}
