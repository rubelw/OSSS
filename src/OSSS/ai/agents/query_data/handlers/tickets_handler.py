...
async def _fetch_tickets(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling tickets API")
        raise QueryDataError(
            f"Error querying tickets API: {e}",
            tickets_url=url,
        ) from e
...
class TicketsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (tickets)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_tickets(skip=skip, limit=limit)
        return {"rows": rows, "tickets": rows}
