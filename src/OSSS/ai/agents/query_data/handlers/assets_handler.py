...
async def _fetch_assets(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/assets"
    ...
    try:
        ...
        logger.exception("Error calling assets API")
        raise QueryDataError(
            f"Error querying assets API: {e}",
            assets_url=url,
        ) from e
...
def _build_assets_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_assets_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
