...
async def _fetch_facilities(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/facilities"
    ...
    try:
        ...
        logger.exception("Error calling facilities API")
        raise QueryDataError(
            f"Error querying facilities API: {e}",
            facilities_url=url,
        ) from e
...
def _build_facilities_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_facilities_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class FacilitiesHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "facilities": rows}
