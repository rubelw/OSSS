...
async def _fetch_meetings(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling meetings API")
        raise QueryDataError(
            f"Error querying meetings API: {e}",
            meetings_url=url,
        ) from e
...
class MeetingsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (meetings)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_meetings(skip=skip, limit=limit)
        return {"rows": rows, "meetings": rows}
