...
async def _fetch_journal_batches(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/journal_batches"
    ...
    try:
        ...
        logger.exception("Error calling journal_batches API")
        raise QueryDataError(
            f"Error querying journal_batches API: {e}",
            journal_batches_url=url,
        ) from e

    if not isinstance(data, list):
        raise QueryDataError(
            f"Unexpected journal_batches payload type: {type(data)!r}",
            journal_batches_url=url,
        )
    return data


def _build_journal_batches_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class JournalBatchesHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "journal_batches": rows}
