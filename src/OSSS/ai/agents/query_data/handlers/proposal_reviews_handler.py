...
async def _fetch_proposal_reviews(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/proposal_reviews"
    ...
    try:
        ...
        logger.exception("Error calling proposal_reviews API")
        raise QueryDataError(
            f"Error querying proposal_reviews API: {e}",
...
class ProposalReviewsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (proposal_reviews)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_proposal_reviews(skip=skip, limit=limit)
        return {"rows": rows, "proposal_reviews": rows}
