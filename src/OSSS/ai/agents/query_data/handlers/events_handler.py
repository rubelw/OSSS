...
async def _fetch_events(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling events API")
        raise QueryDataError(
            f"Error querying events API: {e}",
            events_url=url,
        ) from e
...
def _build_events_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_events_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
