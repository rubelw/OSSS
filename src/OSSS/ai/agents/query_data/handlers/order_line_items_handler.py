...
async def _fetch_order_line_items(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/order_line_items"
    ...
    try:
        ...
        logger.exception("Error calling order_line_items API")
        raise QueryDataError(
            f"Error querying order_line_items API: {e}",
...
class OrderLineItemsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (order_line_items)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_order_line_items(skip=skip, limit=limit)
        return {"rows": rows, "order_line_items": rows}
