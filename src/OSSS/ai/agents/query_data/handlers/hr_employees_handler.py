...
async def _fetch_hr_employees(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/hr_employees"
    ...
    try:
        ...
        logger.exception("Error calling hr_employees API")
        raise QueryDataError(
            f"Error querying hr_employees API: {e}",
...
def _build_hr_employees_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_hr_employees_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class HrEmployeesHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "hr_employees": rows}
