...
async def _fetch_course_sections(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/course_sections"
    ...
    try:
        ...
        logger.exception("Error calling course_sections API")
        raise QueryDataError(
            f"Error querying course_sections API: {e}",
...
def _build_course_sections_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_course_sections_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class CourseSectionsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "course_sections": rows}
