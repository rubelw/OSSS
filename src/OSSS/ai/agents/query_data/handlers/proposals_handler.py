...
async def _fetch_proposals(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling proposals API")
        raise QueryDataError(
            f"Error querying proposals API: {e}",
            proposals_url=url,
        ) from e
...
class ProposalsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (proposals)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_proposals(skip=skip, limit=limit)
        return {"rows": rows, "proposals": rows}
