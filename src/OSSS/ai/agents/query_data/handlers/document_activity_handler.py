...
async def _fetch_document_activity(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/document_activity"
    ...
    try:
        ...
        logger.exception("Error calling document_activity API")
        raise QueryDataError(
            f"Error querying document_activity API: {e}",
...
def _build_document_activity_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_document_activity_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class DocumentActivityHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "document_activity": rows}
