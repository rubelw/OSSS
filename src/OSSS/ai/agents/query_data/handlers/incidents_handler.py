...
async def _fetch_incidents(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling incidents API")
...
def _build_incidents_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class IncidentsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "incidents": rows}
