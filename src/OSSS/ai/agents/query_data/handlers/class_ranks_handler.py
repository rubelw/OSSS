...
async def _fetch_class_ranks(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/class_ranks"
    ...
    try:
        ...
        logger.exception("Error calling class_ranks API")
        raise QueryDataError(
            f"Error querying class_ranks API: {e}",
            class_ranks_url=url,
        ) from e
...
def _build_class_ranks_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_class_ranks_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class ClassRanksHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "class_ranks": rows}
