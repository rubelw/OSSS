...
async def _fetch_policy_versions(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/policy_versions"
    ...
    try:
        ...
        logger.exception("Error calling policy_versions API")
        raise QueryDataError(
            f"Error querying policy_versions API: {e}",
...
def _build_policy_versions_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class PolicyVersionsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (policy_versions)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_policy_versions(skip=skip, limit=limit)
        return {"rows": rows, "policy_versions": rows}
