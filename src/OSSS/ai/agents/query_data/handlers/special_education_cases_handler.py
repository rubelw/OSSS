...
# API Fetch
# -------------------------------------------------------------------
async def _fetch_special_education_cases(
    skip: int = 0,
    limit: int = 100,
) -> List[Dict[str, Any]]:
    """
    Call the OSSS data service for special_education_cases records.

    Returns a list of dicts, each representing one special_education_cases row.
    ...
    try:
        ...
        logger.exception("HTTP error calling special_education_cases API")
        # Keep QueryDataError simple/compatible: single message argument
        raise QueryDataError(
            f"HTTP {status} error querying special_education_cases API: {str(e)}"
        ) from e

    except Exception as e:
        logger.exception("Error calling special_education_cases API")
        raise QueryDataError(
            f"Error querying special_education_cases API: {str(e)}"
        ) from e
...
def _stringify_cell(value: Any, max_len: int = 120) -> str:
    ...
    Prevents huge blobs from blowing up the chat view.
    """
    if value is None:
        return ""
    s = str(value)
...
# Markdown Builder
# -------------------------------------------------------------------
def _build_special_education_cases_markdown_table(
...
class SpecialEducationCasesHandler(QueryHandler):
    ...
    async def fetch(
        ...
        # Expose both the generic "rows" and a typed key
