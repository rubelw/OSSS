...
async def _fetch_accommodations(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/accommodations"
    ...
    try:
        ...
        logger.exception("Error calling accommodations API")
        raise QueryDataError(
            f"Error querying accommodations API: {e}",
...
def _build_accommodations_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_accommodations_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class AccommodationsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "accommodations": rows}
