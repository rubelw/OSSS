...
async def _fetch_behavior_codes(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/behavior_codes"
    ...
    try:
        ...
        logger.exception("Error calling behavior_codes API")
        raise QueryDataError(
            f"Error querying behavior_codes API: {e}",
...
def _build_behavior_codes_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_behavior_codes_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class BehaviorCodesHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "behavior_codes": rows}
