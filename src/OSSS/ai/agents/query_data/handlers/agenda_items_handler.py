...
async def _fetch_agenda_items(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/agenda_items"
    ...
    try:
        ...
        logger.exception("Error calling agenda_items API")
        raise QueryDataError(
            f"Error querying agenda_items API: {e}",
...
def _build_agenda_items_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_agenda_items_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class AgendaItemsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "agenda_items": rows}
