...
async def _fetch_pages(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling pages API")
        raise QueryDataError(
            f"Error querying pages API: {e}",
            pages_url=url,
        ) from e
...
class PagesHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (pages)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_pages(skip=skip, limit=limit)
        return {"rows": rows, "pages": rows}
