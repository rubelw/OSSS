...
async def _fetch_iep_plans(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/iep_plans"
    ...
    try:
        ...
        logger.exception("Error calling iep_plans API")
        raise QueryDataError(
            f"Error querying iep_plans API: {e}",
            iep_plans_url=url,
        ) from e
...
def _build_iep_plans_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class IepPlansHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "iep_plans": rows}
