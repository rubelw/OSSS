...
async def _fetch_employee_earnings(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/employee_earnings"
    ...
    try:
        ...
        logger.exception("Error calling employee_earnings API")
        raise QueryDataError(
            f"Error querying employee_earnings API: {e}",
...
def _build_employee_earnings_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_employee_earnings_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class EmployeeEarningsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "employee_earnings": rows}
