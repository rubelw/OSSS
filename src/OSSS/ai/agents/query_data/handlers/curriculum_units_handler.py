...
async def _fetch_curriculum_units(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/curriculum_units"
    ...
    try:
        ...
        logger.exception("Error calling curriculum_units API")
        raise QueryDataError(
            f"Error querying curriculum_units API: {e}",
...
def _build_curriculum_units_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_curriculum_units_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class CurriculumUnitsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "curriculum_units": rows}
