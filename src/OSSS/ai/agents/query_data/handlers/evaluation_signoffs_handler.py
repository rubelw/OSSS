...
async def _fetch_evaluation_signoffs(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/evaluation_signoffs"
    ...
    try:
        ...
        logger.exception("Error calling evaluation_signoffs API")
        raise QueryDataError(
            f"Error querying evaluation_signoffs API: {e}",
...
def _build_evaluation_signoffs_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_evaluation_signoffs_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class EvaluationSignoffsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "evaluation_signoffs": rows}
