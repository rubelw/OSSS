...
async def _fetch_data_quality_issues(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/data_quality_issues"
    ...
    try:
        ...
        logger.exception("Error calling data_quality_issues API")
        raise QueryDataError(
            f"Error querying data_quality_issues API: {e}",
...
def _build_data_quality_issues_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_data_quality_issues_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class DataQualityIssuesHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "data_quality_issues": rows}
