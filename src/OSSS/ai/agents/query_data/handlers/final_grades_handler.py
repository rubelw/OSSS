...
async def _fetch_final_grades(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/final_grades"
    ...
    try:
        ...
        logger.exception("Error calling final_grades API")
        raise QueryDataError(
            f"Error querying final_grades API: {e}",
...
def _build_final_grades_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_final_grades_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class FinalGradesHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "final_grades": rows}
