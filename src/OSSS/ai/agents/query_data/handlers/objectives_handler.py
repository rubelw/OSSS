...
async def _fetch_objectives(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/objectives"
    ...
    try:
        ...
        logger.exception("Error calling objectives API")
        raise QueryDataError(
            f"Error querying objectives API: {e}",
            objectives_url=url,
        ) from e
...
class ObjectivesHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (objectives)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_objectives(skip=skip, limit=limit)
        return {"rows": rows, "objectives": rows}
