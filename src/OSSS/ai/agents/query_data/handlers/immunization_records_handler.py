...
async def _fetch_immunization_records(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling immunization_records API")
    ...
    if not isinstance(data, list):
        raise QueryDataError(
            f"Unexpected immunization_records payload type: {type(data)!r}",
            immunization_records_url=url,
        )
    return data


def _build_immunization_records_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class ImmunizationRecordsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "immunization_records": rows}
