...
async def _fetch_pay_periods(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/pay_periods"
    ...
    try:
        ...
        logger.exception("Error calling pay_periods API")
        raise QueryDataError(
            f"Error querying pay_periods API: {e}",
            pay_periods_url=url,
        ) from e
...
class PayPeriodsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (pay_periods)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_pay_periods(skip=skip, limit=limit)
        return {"rows": rows, "pay_periods": rows}
