...
async def _fetch_work_order_parts(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling work_order_parts API")
        raise QueryDataError(
            f"Error querying work_order_parts API: {e}",
            work_order_parts_url=url,
        ) from e
...
class WorkOrderPartsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        rows = await _fetch_work_order_parts(skip=skip, limit=limit)
        return {"rows": rows, "work_order_parts": rows}
