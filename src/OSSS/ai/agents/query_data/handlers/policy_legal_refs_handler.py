...
async def _fetch_policy_legal_refs(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/policy_legal_refs"
    ...
    try:
        ...
        logger.exception("Error calling policy_legal_refs API")
        raise QueryDataError(
            f"Error querying policy_legal_refs API: {e}",
...
def _build_policy_legal_refs_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class PolicyLegalRefsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (policy_legal_refs)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_policy_legal_refs(skip=skip, limit=limit)
        return {"rows": rows, "policy_legal_refs": rows}
