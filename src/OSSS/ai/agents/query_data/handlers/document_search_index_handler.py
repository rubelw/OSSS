...
async def _fetch_document_search_index(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/document_search_index"
    ...
    try:
        ...
        logger.exception("Error calling document_search_index API")
        raise QueryDataError(
            f"Error querying document_search_index API: {e}",
...
def _build_document_search_index_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_document_search_index_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class DocumentSearchIndexHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "document_search_index": rows}
