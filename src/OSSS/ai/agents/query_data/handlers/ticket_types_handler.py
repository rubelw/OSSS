...
async def _fetch_ticket_types(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling ticket_types API")
        raise QueryDataError(
            f"Error querying ticket_types API: {e}",
            ticket_types_url=url,
        ) from e
...
class TicketTypesHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (ticket_types)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_ticket_types(skip=skip, limit=limit)
        return {"rows": rows, "ticket_types": rows}
