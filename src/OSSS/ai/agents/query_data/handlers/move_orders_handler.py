...
async def _fetch_move_orders(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/move_orders"
    ...
    try:
        ...
        logger.exception("Error calling move_orders API")
        raise QueryDataError(
            f"Error querying move_orders API: {e}",
            move_orders_url=url,
        ) from e
...
class MoveOrdersHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (move_orders)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_move_orders(skip=skip, limit=limit)
        return {"rows": rows, "move_orders": rows}
