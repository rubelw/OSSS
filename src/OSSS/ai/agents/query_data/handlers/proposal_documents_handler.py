...
async def _fetch_proposal_documents(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/proposal_documents"
    ...
    try:
        ...
        logger.exception("Error calling proposal_documents API")
        raise QueryDataError(
            f"Error querying proposal_documents API: {e}",
...
def _build_proposal_documents_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class ProposalDocumentsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (proposal_documents)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_proposal_documents(skip=skip, limit=limit)
        return {"rows": rows, "proposal_documents": rows}
