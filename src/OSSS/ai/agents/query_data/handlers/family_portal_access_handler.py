...
async def _fetch_family_portal_access(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/family_portal_access"
    ...
    try:
        ...
        logger.exception("Error calling family_portal_access API")
        raise QueryDataError(
            f"Error querying family_portal_access API: {e}",
...
def _build_family_portal_access_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_family_portal_access_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class FamilyPortalAccessHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "family_portal_access": rows}
