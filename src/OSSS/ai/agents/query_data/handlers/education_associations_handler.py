...
async def _fetch_education_associations(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/education_associations"
    ...
    try:
        ...
        logger.exception("Error calling education_associations API")
        raise QueryDataError(
            f"Error querying education_associations API: {e}",
...
def _build_education_associations_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class EducationAssociationsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "education_associations": rows}
