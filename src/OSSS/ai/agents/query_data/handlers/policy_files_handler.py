...
async def _fetch_policy_files(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/policy_files"
    ...
    try:
        ...
        logger.exception("Error calling policy_files API")
        raise QueryDataError(
            f"Error querying policy_files API: {e}",
...
def _build_policy_files_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class PolicyFilesHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (policy_files)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_policy_files(skip=skip, limit=limit)
        return {"rows": rows, "policy_files": rows}
