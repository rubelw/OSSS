...
async def _fetch_student_transportation_assignments(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        raise QueryDataError(
            ...
            student_transportation_assignments_url=url,
        ) from e
...
def _build_student_transportation_assignments_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_student_transportation_assignments_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
