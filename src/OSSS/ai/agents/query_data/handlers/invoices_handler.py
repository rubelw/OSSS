...
async def _fetch_invoices(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/invoices"
    ...
    try:
        ...
        logger.exception("Error calling invoices API")
        raise QueryDataError(
            f"Error querying invoices API: {e}",
            invoices_url=url,
        ) from e
...
def _build_invoices_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
class InvoicesHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "invoices": rows}
