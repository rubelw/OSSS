...
async def _fetch_permissions(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/permissions"
    ...
    try:
        ...
        logger.exception("Error calling permissions API")
        raise QueryDataError(
            f"Error querying permissions API: {e}",
            permissions_url=url,
        ) from e
...
class PermissionsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (permissions)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_permissions(skip=skip, limit=limit)
        return {"rows": rows, "permissions": rows}
