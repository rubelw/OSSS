...
async def _fetch_user_accounts(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.exception("Error calling user_accounts API")
        raise QueryDataError(
            f"Error querying user_accounts API: {e}",
            user_accounts_url=url,
        ) from e
...
class UserAccountsHandler(QueryHandler):
    ...
    source_label = "your DCG OSSS data service (user_accounts)"

    async def fetch(
        self, ctx: AgentContext, skip: int, limit: int
    ) -> FetchResult:
        rows = await _fetch_user_accounts(skip=skip, limit=limit)
        return {"rows": rows, "user_accounts": rows}
