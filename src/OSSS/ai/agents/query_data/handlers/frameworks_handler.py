...
async def _fetch_frameworks(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/frameworks"
    ...
    try:
        ...
        logger.exception("Error calling frameworks API")
        raise QueryDataError(
            f"Error querying frameworks API: {e}",
            frameworks_url=url,
        ) from e
...
def _build_frameworks_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_frameworks_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class FrameworksHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "frameworks": rows}
