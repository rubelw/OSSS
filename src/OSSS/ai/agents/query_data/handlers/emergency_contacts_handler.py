...
async def _fetch_emergency_contacts(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/emergency_contacts"
    ...
    try:
        ...
        logger.exception("Error calling emergency_contacts API")
        raise QueryDataError(
            f"Error querying emergency_contacts API: {e}",
...
def _build_emergency_contacts_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_emergency_contacts_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class EmergencyContactsHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "emergency_contacts": rows}
