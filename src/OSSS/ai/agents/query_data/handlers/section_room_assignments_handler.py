...
async def _fetch_section_room_assignments(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        raise QueryDataError(
            f"Error querying section_room_assignments API: {e}",
            section_room_assignments_url=url,
        ) from e
...
def _build_section_room_assignments_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
