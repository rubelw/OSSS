...
async def _fetch_health_profiles(skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
    url = f"{API_BASE}/api/health_profiles"
    ...
    try:
        ...
        logger.exception("Error calling health_profiles API")
        raise QueryDataError(
            f"Error querying health_profiles API: {e}",
            health_profiles_url=url,
        ) from e

    if not isinstance(data, list):
        raise QueryDataError(
            f"Unexpected health_profiles payload type: {type(data)!r}",
            health_profiles_url=url,
        )
    return data


def _build_health_profiles_markdown_table(rows: List[Dict[str, Any]]) -> str:
    if not rows:
...
def _build_health_profiles_csv(rows: List[Dict[str, Any]]) -> str:
    if not rows:
        return ""
...
class HealthProfilesHandler(QueryHandler):
    ...
    async def fetch(
        ...
        return {"rows": rows, "health_profiles": rows}
