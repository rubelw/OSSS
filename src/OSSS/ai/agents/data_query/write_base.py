# src/OSSS/ai/agents/data_query/write_base.py
...
from .specs_runtime import DataQuerySpec
...
class DataWriteBaseAgent(BaseAgent):
    ...
    def __init__(self, *, data_query: Dict[str, DataQuerySpec]) -> None:
    ...
    async def run(self, context: AgentContext) -> AgentContext:
        ...
        query_name = (cfg.get("data_query") or "").strip().lower()
        if not query_name:
            return self._store(context, {"ok": False, "error": "execution_config.data_query is required"})

        spec = self.data_query.get(query_name)
        if not spec:
            return self._store(context, {"ok": False, "error": f"Unknown data_query: {query_name!r}"})

        if spec.source != "http":
            return self._store(context, {"ok": False, "error": f"{query_name} is not an HTTP data view (source={spec.source})"}, spec=spec)

        if not spec.base_url:
            return self._store(context, {"ok": False, "error": f"{query_name} missing base_url"}, spec=spec)

        path_template: Optional[str] = getattr(spec, self.path_attr, None)
        if not path_template:
            return self._store(context, {"ok": False, "error": f"{query_name} missing {self.path_attr}"}, spec=spec)
    ...
    def _store(self, context: AgentContext, payload: Dict[str, Any], *, spec: Optional[DataQuerySpec] = None) -> AgentContext:
