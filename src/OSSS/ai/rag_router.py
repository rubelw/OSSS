...
# Ensure max_tokens is defined (either passed in or fallback to default value)
max_tokens = locals().get('max_tokens', 1000)  # Default to 1000 if max_tokens is not defined
...
class RAGRequest(BaseModel):
    ...
    text: str = Form(..., description="User's input text")
    ...
    index: Optional[str] = "main"  # index to query, defaults to "main"
    context_id: Optional[str] = None  # Context ID for session tracking
...
async def chat_rag(
    text: str = Form(...),  # The user's text message (required)
    files: List[UploadFile] = File([]),  # Allow an empty list of files
    ...
    logger.info(f"Files: {', '.join([file.filename for file in files])}")
    ...
    # ---- model / params ----
    model = getattr(settings, "DEFAULT_MODEL", "llama3.2-vision").strip()
    temperature = getattr(settings, "TUTOR_TEMPERATURE", 0.1)
    ...
    if intent == "register_new_student":
        logger.info(f"Processing registration for new student with query: {text}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Registration result: {str(result)}")

                registration_run = result.get("registration_run", {})
                registration_status = registration_run.get("status", "Unknown")
                registration_message = registration_run.get("output_preview", "No details available.")

                # Prepare response format as expected in the frontend
                user_response = {
                    "answer": {
                        "message": {
                    ...
                    "retrieved_chunks": []  # Placeholder for chunks if needed
                }

                # Log the result for debugging
                logger.info(f"Registration result: {user_response}")
                return user_response
            else:
                logger.error(f"Failed to register student: {response.text}")
                return {"error": "Registration failed", "details": response.text}
        except requests.exceptions.RequestException as e:
            logger.error(f"Network error: {e}")
            return {"error": "Network error", "details": str(e)}

    # ---- Continue with RAG retrieval if not registration intent ----
    # Proceed with the embedding and retrieval steps as normal

    async with httpx.AsyncClient(timeout=10.0) as client:
        embed_req = {"model": "nomic-embed-text", "prompt": text}
        ...
        # Example query parameter (you would replace this with how you actually receive the index)
        # This could come from request body, headers, or query parameters based on your design
        requested_index = "main"  # Default value for fallback

        # Simulate receiving an index from somewhere, e.g., request or function argument
        # If you have an input (e.g., from query params), you would replace the following line
        index_input = "tutor"  # Just an example of what might come from an API request or elsewhere

        # Choose which additional index to query: main / tutor / agent
        requested_index = (index_input or "main").strip()

        # Check if the requested index is valid, otherwise fall back to "main"
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            base_response = {
                ...
                "retrieved_chunks": retrieved_chunks,  # This is the context you're returning
                "intent": intent,  # The intent detected in the query
            }

            logger.debug("[/ai/chat/rag] base_response: "+str(base_response))
