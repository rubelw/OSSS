...
class ModelCategory(Enum):
    ...
    GPT5 = "llama3.3"  # Latest generation models
    GPT5_MINI = "llama3.3-mini"  # Fast, cost-effective llama3.3 variants
    GPT5_NANO = "llama3.3-nano"  # Ultra-fast llama3.3 for simple tasks
...
class ModelDiscoveryService:
    ...
    FALLBACK_MODELS = {
        ...
        # Removed llama3.3-chat-latest - we don't want chat variants
        "llama3.3-mini": ModelInfo(
            id="llama3.3-mini",
    ...
    AGENT_MODEL_PREFERENCES: Dict[str, Dict[str, Any]] = {
        ...
        "historian": {
            # Use base llama3.3 - no variants needed
    ...
    def _categorize_model(self, model_id: str) -> ModelCategory:
        ...
        if "llama3.3-nano" in model_lower:
            return ModelCategory.GPT5_NANO
        elif "llama3.3-mini" in model_lower:
            return ModelCategory.GPT5_MINI
        elif "llama3.3" in model_lower:
    ...
    def _determine_speed(self, model_id: str, category: ModelCategory) -> ModelSpeed:
        ...
        # Standard models (default for llama3.3)
        ...
        try:
            ...
            for model_data in models_response.data:
                ...
                if model_id in self.FALLBACK_MODELS:
                    ...
                    is_gpt5_family = "llama3.3" in model_id.lower()

                    # Log variant discovery to encourage base model usage
                    if is_variant and is_gpt5_family:
                        self.logger.info(
                            f"Discovered llama3.3 variant '{model_id}'. "
                            f"Consider using base 'llama3.3' model for simplicity."
                        )
                    ...
                    model_info = ModelInfo(
                        ...
                        category=category,
                        speed=speed,
                        context_window=128000 if "llama3.3" in model_id.lower() else 32000,
                        max_output_tokens=(
                            16384 if "llama3.3" in model_id.lower() else 4096
                        ),
                        supports_json_mode=True,
            ...
            # Special logging for llama3.3 family
            gpt5_models = [
                m
            ...
            if gpt5_models:
                self.logger.info(
                    f"✅ llama3.3 family models available: {', '.join([m.id for m in gpt5_models])}"
                )
    ...
    async def get_best_model_for_agent(
        ...
        def rank_model(model: ModelInfo) -> int:
            ...
            is_base_model = model.id.lower() in ["llama3.3", "llama3.3-nano", "llama3.3-mini"]
    ...
    def get_model_info(self, model_id: str) -> Optional[ModelInfo]:
        ...
        # This handles cases like llama3.3-chat-latest that are discovered but not in fallback
        category = self._categorize_model(model_id)
        speed = self._determine_speed(model_id, category)
        capabilities = self._extract_capabilities(model_id, category)
