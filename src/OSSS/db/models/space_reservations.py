...
class SpaceReservation(UUIDMixin, Base):
    ...
    NOTE: ClassVar[str] =     (
        ...
        "description=Stores space reservations records for the application. "
        "References related entities via: booked by user, space. "
    ...
    __table_args__ = {
        "comment":         (
            "Stores space reservations records for the application. "
            "References related entities via: booked by user, space. "
            "Includes standard audit timestamps (created_at, updated_at). "
            "11 column(s) defined. "
            "Primary key is `id`. "
    ...
    setup = sa.Column(sa.JSON, nullable=True)
