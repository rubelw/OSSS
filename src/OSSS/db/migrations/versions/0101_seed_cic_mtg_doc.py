...
# ---- Alembic identifiers ----
revision = "0101_seed_cic_mtg_doc"
down_revision = "0100_seed_cic_ag_itm"  # update if needed
branch_labels = None
depends_on = None
...
# --- Config / knobs ---
CSV_FILENAME = "cic_meeting_documents.csv"
DEFAULT_ROW_COUNT = int(os.getenv("CIC_MEETING_DOC_ROWS", "300"))
SEED = os.getenv("CIC_MEETING_DOC_SEED")  # e.g. "42" for reproducibility
...
def _generate_rows(
    ...
    if not meeting_ids:
        raise RuntimeError("No cic_meetings found. Cannot generate cic_meeting_documents.")
    if not document_ids:
        ...
        raise RuntimeError("No documents found. Cannot generate cic_meeting_documents.")
...
def upgrade():
    ...
    log.info("[0101] cic_meeting_documents: fetching reference data…")
    meeting_ids, document_ids = _fetch_reference_data(conn)
    log.info("[0101] cic_meeting_documents: %d meetings, %d documents found", len(meeting_ids), len(document_ids))

    log.info("[0101] cic_meeting_documents: generating %d rows…", DEFAULT_ROW_COUNT)
    rows = _generate_rows(meeting_ids, document_ids, DEFAULT_ROW_COUNT)

    csv_path = _csv_path()
    log.info("[0101] cic_meeting_documents: writing CSV at %s (%d rows)…", csv_path, len(rows))
    _write_csv(csv_path, rows)

    # Be idempotent within this migration: clear table, then insert.
    log.info("[0101] cic_meeting_documents: clearing table…")
    conn.execute(sa.text("DELETE FROM cic_meeting_documents"))
    ...
    log.info("[0101] cic_meeting_documents: inserting %d rows…", len(to_insert))
    CHUNK = 1000
    for i in range(0, len(to_insert), CHUNK):
        op.bulk_insert(table, to_insert[i : i + CHUNK])
    log.info("[0101] cic_meeting_documents: done.")

def downgrade():
    conn = op.get_bind()
    log.info("[0101] cic_meeting_documents: deleting all rows (downgrade)…")
    conn.execute(sa.text("DELETE FROM cic_meeting_documents"))