...
class FileConfig(BaseModel):
    ...
    notes_directory: str = Field(
        ...
        min_length=1,
        json_schema_extra={"example": "./src/osss-logs/notes"},
    )
    logs_directory: str = Field(
        "./src/osss-logs/logs",
        description="Directory path for storing log files",
        min_length=1,
        json_schema_extra={"example": "./src/osss-logs/logs"},
    )
...
class ApplicationConfig(BaseModel):
    ...
    def from_env(cls) -> "ApplicationConfig":
        ...
        config.files.notes_directory = os.getenv(
            "OSSS_NOTES_DIR", "./src/osss-logs/notes"
        )
        config.files.logs_directory = os.getenv(
            "OSSS_LOGS_DIR", "./src/osss-logs/logs"
        )
