...
class FastAPIKeycloak:
    ...
    def open_id_configuration(self) -> dict:
        """Returns Keycloak’s OpenID Connect configuration (tries modern path first, then legacy)."""
        errors = []
        candidates = [
            f"{self.realm_uri}/.well-known/openid-configuration",  # modern (no /auth)
            f"{self.server_url}/auth/realms/{self.realm}/.well-known/openid-configuration",  # legacy
        ]
            ...
            if location:
                try:
                    ...
                    if 200 <= direct.status_code < 300:
                        return KeycloakGroup.`model_validate(direct.json())
                except Exception:
                    pass
                    ...
                    if 200 <= children_resp.status_code < 300:
                        for child in children_resp.json():
                            if child.get("name") == group_name:
                                return KeycloakGroup.parse_obj(child)
...
# Expose for `uvicorn OSSS.api:app --reload --app-dir src`
app = create_app()
